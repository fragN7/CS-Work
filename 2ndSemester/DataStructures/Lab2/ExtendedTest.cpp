...
void testAdd() {
	...
	try {
		ListIterator it1 = getIterator(2, list);
		IteratedList::getElement(it1);
		assert(false);
	}
	catch (exception&) {
		assert(true);
...
void testQuantity() {
	...
	for (int i = 0; i < l2.size(); i++) {
		...
		TElem elem = IteratedList::getElement(it);
		assert(l2.search(elem).getCurrent() == it.getCurrent());
	}
	testIteratorSteps(l2);
	for (int i = -30000; i < 300000; i++) {
		ListIterator position = l2.search(i);
		if (position.valid()) {
			assert(l2.remove(position) == i);
		}
	}
	testIteratorSteps(l2);
}
...
void testRemove() {
	...
	for (int i = 0; i < 50; i++) {
        ListIterator it = getIterator(position, list2);
        list2.remove(it);
        compareListMinusPosition(list1, list2, position);
        ListIterator it2 = getIterator(position, list1);
        list1.remove(it2);
        position = (position + 7) % list2.size();
    }
