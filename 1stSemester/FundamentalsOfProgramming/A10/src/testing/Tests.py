...
from src.domain.IterableStructure import IterableStructure, gnome_sort, filtering
...
class TestRepo(unittest.TestCase):
    ...
    def testStudentRepo(self):
        ...
        with self.assertRaises(RepoError) as re:
            self.__repo_student.find_id(some_id)
...
class TestIterable(unittest.TestCase):
    ...
    def testIT(self):
        ...
        self.structure.__delitem__(0)
        self.assertEqual(self.structure.get_list(), [])

        with self.assertRaises(IndexError) as ie:
            self.structure.__delitem__(1)
        self.assertEqual(str(ie.exception), "list assignment index out of range")

        with self.assertRaises(StopIteration) as si:
            self.structure.__next__()
        self.assertEqual(str(si.exception), "")
        ...
        structures = self.structure.get_list()
        self.assertEqual(structures, [1, 1, 1, 1])

        item = self.structure.__iter__()
        self.assertIsNot(item, list)

        next_item = self.structure.__next__()
        self.assertIsNot(next_item, list)

        other_item = self.structure.__getitem__(0)
        self.assertEqual(other_item, 1)

        self.structure.__setitem__(0, 2)
        for index in range(len(self.structure)):
            if not index:
                self.assertEqual(self.structure[index], 2)

        length = self.structure.__len__()
...
class TestSortFilter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.list1 = [1, 2, 10, 9]
        self.list2 = []
        self.list3 = "no list haha"
        self.list4 = [0]

    @staticmethod
    def filtering_criteria(number):
        return number != 4

    def testFilter(self):
        the_list = filtering(self.list, self.filtering_criteria)
        self.assertEqual(the_list, [1, 2, 3, 5])

        the_list = filtering(self.list1, self.filtering_criteria)
        self.assertEqual(the_list, [1, 2, 10, 9])
    ...
    def testSort(self):
        gnome_sort(self.list, self.greater)
        self.assertEqual(self.list, [5, 4, 3, 2, 1])

        with self.assertRaises(IndexError) as ie:
            gnome_sort(self.list2, self.greater)
        self.assertEqual(str(ie.exception), "Can not sort null list")

        with self.assertRaises(TypeError) as te:
            gnome_sort(self.list3, self.greater)
        self.assertEqual(str(te.exception), "'str' object does not support item assignment")

        something = gnome_sort(self.list4, self.greater)
