...
CREATE TABLE ChessTitle(
	tid TINYINT IDENTITY PRIMARY KEY,
	title_name CHAR(15)
);
...
CREATE TABLE ClubPlayers(
	centry_id INT IDENTITY PRIMARY KEY,
	club_id INT,
	player_id INT,
...
CREATE TABLE Tournament(
	tourn_id INT IDENTITY PRIMARY KEY,
...
CREATE TABLE FideStandings(
	rank_id INT IDENTITY PRIMARY KEY,
...
CREATE TABLE WorldChampions(
	worldc_id TINYINT IDENTITY PRIMARY KEY,
	player_id INT REFERENCES ChessPlayer(player_id),
	years TINYINT DEFAULT 'TBA'
);

CREATE TABLE Tactic (
	tactic_id INT IDENTITY PRIMARY KEY,
